...
const AuthScreen: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If the user arrived via an invite link, a roomId was stashed in the
      // store before auth. Send them straight to the lobby so the join fires
      // automatically instead of dropping them on the main menu.
      const pendingRoom = useGameStore.getState().pendingJoinRoomId;
      setScreen(pendingRoom ? 'lobby' : 'menu');
    } catch (err: unknown) {
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // ── Parse invite URL ───────────────────────────────────────
    // Handles two link formats produced by the app:
    //   /join/{roomId}          — generated by InviteModal / InviteLink
    //   /join?code={inviteCode} — generated by the server's invite API
    const resolveInviteUrl = async (): Promise<string | null> => {
      const { pathname, search } = window.location;

      // Format 1: /join/{roomId}  (direct room link)
      const directMatch = pathname.match(/^\/join\/([^/?#]+)/);
      if (directMatch) {
        window.history.replaceState({}, '', '/');
        return directMatch[1];
      }

      // Format 2: /join?code={inviteCode}  (server-generated invite)
      if (pathname === '/join') {
        const code = new URLSearchParams(search).get('code');
        if (code) {
          window.history.replaceState({}, '', '/');
          try {
            const res = await fetch(`/api/invites/${code}/resolve`);
            if (res.ok) {
              const data = await res.json();
    ...
    resolveInviteUrl().then((roomId) => {
      if (roomId) {
        // Stash the room ID so the Lobby can emit join_room once the socket connects.
        useGameStore.getState().setPendingJoinRoomId(roomId);
      }

      restoreSession().then((authUser) => {
        if (authUser) {
          // Session found — go straight to lobby when following an invite link,
          // otherwise land on the main menu as usual.
          useGameStore.getState().setScreen(roomId ? 'lobby' : 'menu');
        }
